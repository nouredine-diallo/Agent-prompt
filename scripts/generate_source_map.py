...
def find_source(txt_path):
    ...
    for sub in ["downloads", "arxiv", "github"]:
        for root, dirs, files in os.walk(os.path.join("docs", sub)):
            for f in files:
...
def guess_url_and_author(txt_path, source_path):
    ...
    # TODO: enrichir avec d'autres heuristiques
    return url, author
